#!/usr/bin/env python3
"""Generate supabase/migrations/20260913_seed_convention_blog.sql from embedded post data."""

from pathlib import Path

IMG = "https://images.unsplash.com/photo-1438032005730-c779502df39b?w=1200&h=630&fit=crop"

POSTS = [
    {
        "id": "f1f1e001-0001-4001-8001-000000000001",
        "title": "Failure: The Pathway to Success",
        "slug": "failure-the-pathway-to-success",
        "excerpt": "Failure is not the end — it is an event, an experience, and a springboard when we learn, respond rightly, and trust God to raise us again.",
        "category": "Education",
...
<p>Success is not served on a platter of gold — it is built through perseverance, obedience, and faith in Christ who is our All in All.</p>""",
    ...
    {
        "id": "f1f1e002-0002-4002-8002-000000000002",
        "title": "The Importance of Time to Youth",
        "slug": "importance-of-time-to-youth",
        "excerpt": "Time is a gift from God. Youth is the season to redeem the days, seek salvation, and build a life of wisdom and purpose.",
        "category": "Youth",
        "author": "Fire-Fire Youth Ministry",
...
<p><em>"And let us not be weary in well doing: for in due season we shall reap, if we faint not"</em> (Galatians 6:9).</p>""",
    ...
    {
        "id": "f1f1e003-0003-4003-8003-000000000003",
        "title": "The Way, the Truth, and the Life",
        "slug": "the-way-truth-and-life",
        "excerpt": "In perilous times, the world searches for direction, reality, and eternal life. Jesus Christ alone is the Way, the Truth, and the Life.",
        "category": "Teaching",
...
<p>Turn to God today. Open life's instruction manual — the Holy Bible — and let Jesus Christ be your All in All.</p>""",
    ...
    {
        "id": "f1f1e004-0004-4004-8004-000000000004",
        "title": "Jesus Christ: All in All",
        "slug": "jesus-christ-all-in-all",
        "excerpt": "He is the Redeemer, Justifier, Provider, Deliverer, and King. At the end of it all, you will discover He is truly All in All.",
        "category": "Worship",
        "author": "Fire-Fire International Evangelical Church",
...
<p><em>Theme: Fire-Fire International Evangelical Church Convention 2026 — April 2–6, Olomi, Ibadan.</em></p>""",
    ...
    {
        "id": "f1f1e005-0005-4005-8005-000000000005",
        "title": "Top 10 Things You Did Not Learn in High School or College",
        "slug": "top-10-things-not-learned-in-school",
        "excerpt": "Life lessons from Psalm 23 — provision, satisfaction, renewal, direction, correction, preparation, and completion through the Good Shepherd.",
        "category": "Education",
        "author": "Fire-Fire Youth Ministry",
...
<li><strong>Dependability brings greater responsibility.</strong> Rewards go to finishers. Trust His perfect completion (Psalm 23:6).</li>
    ...
    {
        "id": "f1f1e006-0006-4006-8006-000000000006",
        "title": "Stepping Out Again",
        "slug": "stepping-out-again",
        "excerpt": "The greatest enemy is not failure — it is the fear of trying again. God is the God of second chances.",
        "category": "Faith",
        "author": "Fire-Fire Youth Ministry",
        "image": IMG,
        "featured": False,
...
def main() -> None:
    out = Path(__file__).resolve().parents[1] / "supabase/migrations/20260913_seed_convention_blog.sql"
    lines = [
    ...
    for idx, post in enumerate(POSTS):
        ...
        lines.extend(
            [
                ...
                ") values (",
                f"  '{post['id']}'::uuid,",
                f"  '{sql_quote(post['title'])}',",
                f"  '{sql_quote(post['excerpt'])}',",
                f"  ${tag}${post['content']}${tag}$,",
                f"  '{sql_quote(post['author'])}',",
                f"  '{sql_quote(post['category'])}',",
                f"  '{sql_quote(post['image'])}',",
                f"  {'true' if post['featured'] else 'false'},",
                "  true,",
                f"  '{sql_quote(post['slug'])}',",
                "  'published',",
                f"  now() - interval '{idx} days',",
                f"  '{sql_quote(post['tags'])}',",
                f"  {idx},",
                f"  now() - interval '{idx} days',",
                "  now()",
                ")",
