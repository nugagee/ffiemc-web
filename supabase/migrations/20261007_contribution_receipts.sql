-- Optional receipt upload on media contributions + keep submit/admin RPCs in sync.
...
begin
  ...
  insert into public.social_media_contributions (
    month_id, team_member_id, full_name, amount, note, source, receipt_url
  ) values (
    v_month.id,
    p_team_member_id,
    v_name,
...
grant execute on function public.submit_media_contribution(text, uuid, text, numeric, text, text)
...
begin
  ...
  if p_id is null then
    ...
    insert into public.social_media_contributions (
      ...
      coalesce(trim(p_data->>'note'), ''), 'admin', v_receipt
    )
...
-- Include receipt on public report contribution rows
