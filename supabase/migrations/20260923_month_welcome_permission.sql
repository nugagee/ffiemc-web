-- Allow home.monthWelcome in permission catalog (month welcome popup toggle).
...
begin
  ...
  -- Month welcome is edited from Banners; allow banners editors as well as home.monthWelcome
  if p_page = 'home' and p_section = 'monthWelcome' then
    v_admin := public._require_admin(p_token);
    if not (
      public._has_perm(v_admin, 'home.monthWelcome', 'edit')
      or public._has_perm(v_admin, 'banners', 'edit')
      or public._has_perm(v_admin, 'home.announcements', 'edit')
    ) then
      raise exception 'You do not have permission to edit home.monthWelcome';
    end if;
  else
